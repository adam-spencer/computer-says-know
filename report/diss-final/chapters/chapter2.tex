...
\begin{itemize}
        \item $Q$ is the \emph{Query} vector,
        \item $K$ is the \emph{Key} vector, and
        \item $V$ is the \emph{Value} vector.
\end{itemize}
...
\begin{itemize}
        ...
        In this layer, $Q$, $K$, and $V$ are all calculated by multiplying an input embedding (a vector representation of the raw input sequence) by a three different trained weight vectors.
...
For the sake of understanding a simplified version of the operation of the transformeris as follows;

\begin{enumerate}
        \item An input sequence is input into the 'encoder stack' (the layers which make up the encoder) after being modified using 'positional embeddings', which act to preserve information about the position of each part of the input sequence.
        \item Each self-attention layer in the encoder 
\end{enumerate}
